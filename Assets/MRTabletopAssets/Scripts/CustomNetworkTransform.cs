...
namespace Unity.Netcode.Components
{
    ...
    [AddComponentMenu("Netcode/" + nameof(CustomNetworkTransform))]
    ...
    public class CustomNetworkTransform : NetworkBehaviour
    {
        ...
        protected NetworkManager m_CachedNetworkManager => NetworkManager.Singleton;

        [Header("Physics Settings")]

        [SerializeField]
        [Tooltip("If Rigidbody is present, transform will be moved via the rigidbody. Not used if syncing in local space.")]
        Rigidbody m_Rigidbody = null;

        protected bool m_UseRigidbody = false;
        ...
        protected void TryCommit(bool isDirty)
        {
        ...
        public void ResetLocalAuthoritativeStateToCurrentTransform()
        {
            ...
            m_LocalAuthoritativeNetworkState.PositionX = position.x;
            m_LocalAuthoritativeNetworkState.PositionY = position.y;
            m_LocalAuthoritativeNetworkState.PositionZ = position.z;

            m_LocalAuthoritativeNetworkState.RotAngleX = rotAngles.x;
            m_LocalAuthoritativeNetworkState.RotAngleY = rotAngles.y;
            m_LocalAuthoritativeNetworkState.RotAngleZ = rotAngles.z;

            m_LocalAuthoritativeNetworkState.ScaleX = scale.x;
            m_LocalAuthoritativeNetworkState.ScaleY = scale.y;
            m_LocalAuthoritativeNetworkState.ScaleZ = scale.z;
        ...
        private void ApplyInterpolatedNetworkStateToTransform(NetworkTransformState networkState, Transform transformToUpdate)
        {
            ...
            if (SyncPositionX || SyncPositionY || SyncPositionZ)
            {
                ...
                else
                {
                    ...
                    {
                        m_Rigidbody.MovePosition(interpolatedPosition);
                    }
                }
            ...
            if (SyncRotAngleX || SyncRotAngleY || SyncRotAngleZ)
            {
                ...
                else
                {
                    ...
                    {
                        m_Rigidbody.MoveRotation(Quaternion.Euler(interpolatedRotAngles));
                    }
                }
...
#if DEVELOPER_MODE
            SetInterpolateModeInternal(SmartUXNetworkManager.NetworkTransformInterpolating);
            SmartUXNetworkManager.NetworkInterpolateChanged += SetInterpolateModeInternal;
#endif
        ...
        {
            m_UseRigidbody = m_Rigidbody != null;
...
#if DEVELOPER_MODE
        void SetInterpolateModeInternal(bool useInterpolate)
        {
