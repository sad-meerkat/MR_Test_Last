...
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────┐  ┌───────────┐ │
│  │    Lobby API     │  │    Relay API     │  │Auth(익명/계정)│  │Vivox(음성)│ │
│  └────────▲─────────┘  └────────▲─────────┘  └──────▲───────┘  └───▲───────┘ │
└───────────┼─────────────────────┼───────────────────┼──────────────┼─────────┘
            │                     │                   │              │
┌───────────▼─────────────────────▼───────────────────▼──────────────▼─────────┐
...
│   - 게임 상태의 '진실(Ground Truth)' 결정 및 동기화 주도                       │
│   - 승패 판정, 캐릭터 실시간 체력(HP) 계산, 타이머 연산 전담                    │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                 Netcode for GameObjects (NGO) 레이어                │     │
│  │    [NetworkVariable] 실시간 상단 체력바(HP), 타이머, 룸 상태 동기화    │    │
│  │    [RPC System] 공격(ServerRpc) ──► 피격 판정 및 상대 캐릭터 제어     │     │
│  └──────────────────────────────────▲──────────────────────────────────┘    │ 
└─────────────────────────────────────┼───────────────────────────────────────┘
                                      │ (실시간 데이터 동기화 패킷 전송)
┌─────────────────────────────────────▼───────────────────────────────────────┐
│                                                                             │
│  🎮 CLIENT PLAYER (참가자)                                                  │
│  │- 자신의 입력 데이터(오른손 가속도, 왼손 조이스틱 토크)를 Host로 전송          │
│  │- Host로부터 결정된 'Ground Truth' 데이터를 수신하여 3인칭 아바타 화면 렌더링 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
비대칭 개별 감지: 양손의 컨트롤러 활성 플래그와 핸드 트래킹 상태를 독립 연산합니다. 왼손으로는 물리 컨트롤러의 조이스틱을 움직여 캐릭터를 부드럽게 조작(Locomotion)하고, 오른손은 컨트롤러를 내려놓고 맨손(Hand Tracking)으로 주술 영창 제스처 및 검 휘두르기 액션을 구사하는 혁신적인 하이브리드 비대칭 조작계를 구축했습니다.
...
2.  테이블탑 동적 물리 경계 (Dynamic Tabletop Physics Boundary)
...
3.  가상 터치 인터랙션 및 철권 룰 체인 (Battle System & UI)
...
📁 Key Files & Architecture (주요 소스 코드 구조)
Assets/
├── Scripts/
│   ├── Multiplayer/
│   │   ├── OVRMultiplayerBootstrap.cs    # UGS 서버 인프라 및 런타임 로비 캔버스 동적 빌더
│   │   └── XRINetworkPlayer.cs           # NGO 기반 플레이어 헤드/양손 트랜스폼 실시간 동기화 코어
│   └── Interaction/
│       ├── IndependentHandModalityManager.cs # 양손 독립형 모달리티(컨트롤러/핸드) 상태 추적 및 제어 브릿지
│       ├── MetaToXRIBridge.cs            # OVRCameraRig 하드웨어 좌표를 XRI 타겟으로 주입하는 어셈블리 헬퍼
│       ├── TableBoundary.cs              # 가상 스테이지 평면 기준 투명 충돌 보더라인 콜라이더 실시간 생성기
│       └── VRButtonTouch.cs              # 핸드 콜라이더 접촉 기반 런타임 트리거 버튼 컴포넌트
└── Scenes/
    └── 핸즈 스킬 적용 2.unity           # 메인 대전 격투 시뮬레이션 및 멀티플레이 통합 테스트 씬
    └── VR F Test .unity                # 손 제스쳐를 활용한 애니메이션 명장면 체험 씬
